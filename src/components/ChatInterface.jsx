...
export function ChatInterface() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch jobs
        const jobsResponse = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5001'}/api/jobs`);
        if (jobsResponse.ok) {
        ...
        const companiesResponse = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5001'}/api/companies`);
        if (companiesResponse.ok) {
          const companiesResult = await companiesResponse.json();
          setCompaniesData(companiesResult.companies || []);
        } else {
          // Try alternative endpoint
          const altCompaniesResponse = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5001'}/api/company`);
